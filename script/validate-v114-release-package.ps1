...
if ($repositoryVersion -cne $expectedVersion) {
    throw "v1.1.4 package validation requires version.txt to remain exactly $expectedVersion."
}
...
function Assert-WindowsBinaryArchitecture {
    ...
    elseif ($ExpectedRuntimeIdentifier -ceq 'win-x64') {
        ...
        throw "Unsupported v1.1.4 Windows runtime identifier: $ExpectedRuntimeIdentifier"
    }
...
try {
    ...
    if ($PackageKind -ceq 'zip') {
        ...
        if ($RuntimeIdentifier -notin @('linux-x64', 'linux-arm64')) {
            throw "Unsupported v1.1.4 Linux runtime identifier: $RuntimeIdentifier"
        }
