param(
    ...
    [Parameter(Mandatory = $true)] [string]$ObserverScript,
    [Parameter(Mandatory = $true)] [string]$DumpTool,
    [Parameter(Mandatory = $true)] [string]$OutputDirectory,
    [ValidateRange(1, 10000)] [int]$MaxIterations = 2000,
    [ValidateRange(1, 240)] [int]$DurationMinutes = 70,
...
function Set-SafeHistoricalEnvironment {
    ...
    foreach ($entry in [Environment]::GetEnvironmentVariables().GetEnumerator()) {
        ...
        if ($forbiddenTokens | Where-Object { $upper.Contains($_, [StringComparison]::Ordinal) }) { continue }
...
for ($iteration = 1; $iteration -le $MaxIterations; $iteration++) {
    ...
    try {
        if (-not $observer.Start()) { throw 'Historical-pipe observer did not start.' }
        ...
        if (-not $target.Start()) { throw 'Historical-pipe target did not start.' }
        [IO.File]::WriteAllText($pidPath, [string]$target.Id, [Text.UTF8Encoding]::new($false))

        # Match historical Invoke-IsolatedProcess stream ownership exactly: create the
        # two ReadToEndAsync drains immediately, then poll WaitForExit(25).
        $stdoutTask = $target.StandardOutput.ReadToEndAsync()
        $stderrTask = $target.StandardError.ReadToEndAsync()
        $targetDeadline = [DateTimeOffset]::UtcNow.AddSeconds($TimeoutSeconds)
        while (-not $target.WaitForExit(25)) {
            if ([DateTimeOffset]::UtcNow -ge $targetDeadline) {
                throw 'Historical-pipe target exceeded phase timeout before normal exit.'
            }
        }
        $target.WaitForExit()
        ...
        if (-not $observer.WaitForExit(($TimeoutSeconds + 150) * 1000)) {
            ...
            throw 'Historical-pipe observer exceeded fail-closed completion window.'
        }
        ...
        if (-not (Test-Path -LiteralPath $resultPath -PathType Leaf)) {
            throw 'Historical-pipe observer did not publish observer-result.json.'
        }
        $result = Get-Content -LiteralPath $resultPath -Raw | ConvertFrom-Json
        $results.Add($result)
        if ($observer.ExitCode -ne 0 -or -not $result.executionValid -or $result.watchdogSeen) { break }
    }
    finally {
...
$summary = [ordered]@{
    ...
    phase = 'assembly-info'
    launchTopology = 'workflow-step pwsh -> direct System.Diagnostics.Process target; sibling observer'
    pipeDrain = 'historical ReadToEndAsync(stdout/stderr) + WaitForExit(25)'
    targetInvocation = 'dotnet DownKyi.Desktop.Tests.dll -assemblyInfo'
    iterations = $results.Count
    jsonDetections = @($results | Where-Object { $_.assemblyInfoJsonSeen }).Count
    ...
    alive500msAfterStart = @($results | Where-Object { $_.alive500msAfterStart }).Count
