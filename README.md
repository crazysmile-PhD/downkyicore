...
程序会缓存已经读取的登录 Cookie。运行期间从外部创建、修改、删除或替换 `Login` 文件时，DownKyi 会使现有缓存失效，并在下一次使用登录 Cookie 时重新读取该文件，无需重新启动。如果操作系统无法启动文件监视，DownKyi 会记录错误并继续正常启动，但本次运行不会自动刷新外部修改。
