...
contracts:
  ...
  - Danmaku enumeration first reads `DmWebViewReply.dm_sge.total`, rejects bounds that the inclusive `int` loop cannot traverse safely, then requests exactly segments `1..total`; an empty interior segment is valid and cannot act as EOF. Missing/invalid metadata, HTTP, IO and malformed protobuf responses remain failures. Main cover retains transfer key `cover`, while page cover uses `page-cover`, so neither durable path overwrites the other.
  - Artifact paths are claimed through the existing Application task owner before their first write. A retry that reuses a logical artifact key for a different physical path appends a deterministic durable claim instead of replacing the prior owner; task state and SQLite remain the only ownership truth.
