...
contracts:
  ...
  - `ValidateForensics` proves that an observer which misses a reparented descendant cannot make the ownership gate green, and that an injected observer failure remains visible beside `ResidualChildProcess` while lease-owned cleanup completes.
  - The Architecture gate scans the forensics observer's transitive PowerShell helper closure; only a diagnostic collector cleanup helper may terminate the collector it created, and helper-authority mutations must fail the review-invariant gate.
