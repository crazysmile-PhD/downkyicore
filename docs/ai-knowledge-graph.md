...
contracts:
  ...
  - Temporary cleanup is identity-bound and content-independent; final cleanup requires persisted identity, length, and hash. Windows validates and marks deletion through the same live handle, so a pathname replacement cannot transfer deletion authority to a foreign object. Linux and macOS currently preserve temporary and final outputs when authoritative object identity is unavailable; no pathname-delete fallback is allowed.
