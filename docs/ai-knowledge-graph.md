...
contracts:
  ...
  - Each Windows and Linux RID performs one dotnet publish into a fixed canonical directory containing non-empty DownKyi, aria2, FFmpeg, ffprobe, dependency-manifest, and LICENSE files; PupNet standalone packaging copies that same validated payload into its package staging directory.
  ...
  - Linux ARM64 is cross-published and packaged on an x64 Ubuntu host, then transported by tar to a native ARM64 validator before final artifact upload; the final aarch64 application, bundled media tools, AppImage runtime, and deb architecture remain fail-closed.
