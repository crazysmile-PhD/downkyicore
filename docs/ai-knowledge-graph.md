...
- `src/DownKyi.Infrastructure` currently owns SQLite task persistence, write-behind, clock, the injected Bilibili HTTP/buvid runtime, the filesystem physical output path resolver, and the logging sink/retention/export runtime. Aria2, FFmpeg and remaining filesystem paths remain mainly in `DownKyi.Core`.
...
- Workers and pipeline entry points use `DownloadTaskId`. `DownloadPipeline` is a typed stage sequencer; `DownloadExecutionContextFactory` creates one immutable execution input from the committed Domain snapshot and current settings, plus the initial `PlayUrl` reference. Localized activity rendering and completion-list mutation have separate Desktop owners that resolve UI projections by task ID. `DownloadListState` privately owns mutable collections and exposes stable `ReadOnlyObservableCollection<T>` projections. Bilibili HTTP uses injected Application ports with an async Infrastructure implementation; the old static/synchronous compatibility layer is gone.
...
contracts:
  ...
  - `DownloadExecutionContext` owns one immutable `DownloadExecutionInput` built from the committed Domain snapshot and current settings, plus mutable per-run playback and output results. It accepts the current operation token at each active check and cannot retain a short-lived command token.
...
hazards:
  ...
  - Initial `PlayUrl` is the only execution decision input sourced from the UI projection. The factory captures its reference once, so later property replacement cannot affect the run; a future writer that mutates the same `PlayUrl` object in place would require a stronger immutable playback boundary.
  ...
  - Media and artifact stages must not reacquire `DownloadingItem` for execution decisions. Only the activity presenter and completion projector may resolve the current projection by `DownloadTaskId` to update UI state.
...
contracts:
  ...
  - Linux ARM64 is cross-published and packaged on an x64 Ubuntu host, then transported by tar to a native ARM64 validator before final artifact upload; every ELF in the extracted runtime, including native shared libraries, must be aarch64, while AppImage runtime and deb architecture remain fail-closed.
  - AppImage content is observed with native `--appimage-extract`, and an AppRun symlink must target the existing executable `usr/bin/DownKyi`; both AppRun and `--appimage-extract-and-run` must remain alive for the launch-smoke window.
