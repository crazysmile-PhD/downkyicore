...
| `passport.bilibili.com/x/passport-login/web/qrcode/poll` | `LoginQr.GetLoginStatus`; poll QR state and capture the login response cookies | GET, `data`, generated key required; successful response may set session cookies | `fixed/current`; NEMO login map agrees; full live poll intentionally not persisted | The same isolated session owns generate, poll and HTTPS Bilibili callback hops. The `passport.biligame.com/x/passport-login/web/crossDomain` landing address terminates callback traversal without receiving a request. Poll, callback and landing-query cookies are merged, atomically persisted, reloaded and accepted only after `/nav` returns `isLogin=true`; failed validation restores the previous file. `BilibiliLoginSessionTests`, `LoginHelperTests` and `LoginCoordinatorTests` cover the deterministic flow. |
