...
- Replace the remaining static `LogManager` usage with injected `Microsoft.Extensions.Logging`; Prism/Host composition, App crash/shutdown, About log management, download runtime/file lifecycle/maintenance, FFmpeg, aria2, validated settings persistence, and legacy database migration are migrated, with download task scopes using redacted short IDs; 29 production caller files still contain executable static logger references.
