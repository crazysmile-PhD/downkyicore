...
- Reduce `App.axaml.cs` to XAML, Host, shell, start, and stop; static download collections and download-runtime storage lookup are removed, while Prism composition and download bootstrap still remain.
- Move the remaining download bootstrap out of `App.axaml.cs`; `DownloadListState` now owns stable downloading/history projections, storage maintenance and bounded async shutdown are Host-owned/awaited, and shutdown cancellation drains workers before resumable-state recovery.
