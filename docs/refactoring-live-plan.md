...
- Replace ViewModel `Task.Run` calls with page-specific cancellable use cases; 1 active call site remains in the legacy upgrade dialog, excluding comments, after video-detail, user-space, Bili Helper, account, friend relation, season/series, favorites, personal media, publication, my-space, and bangumi-follow work moved to coordinators. User-space API tokens now reach the HTTP boundary, list projection is batched, and all media pages share directory-cancel-safe download coordination.
