...
- Replace ViewModel `Task.Run` calls with page-specific cancellable use cases; 11 active call sites remain, excluding comments, after video-detail, user-space, Bili Helper, account, friend relation, season/series, and private/public favorites work moved to coordinators. Favorites now return immutable snapshots, batch UI projection, share one cancellable add-to-download loop, and stop immediately when directory selection is canceled.
