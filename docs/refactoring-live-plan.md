...
- Replace ViewModel `Task.Run` calls with page-specific cancellable use cases; 3 active call sites remain, excluding comments, after video-detail, user-space, Bili Helper, account, friend relation, season/series, favorites, personal media, publication, and my-space work moved to coordinators. User-space API tokens now reach the HTTP boundary; publication projection is batched, while my-space renders its profile before optional balance/relation statistics complete.
