...
- Replace ViewModel `Task.Run` calls with page-specific cancellable use cases; 7 active call sites remain, excluding comments, after video-detail, user-space, Bili Helper, account, friend relation, season/series, favorites, history, and watch-later work moved to coordinators. Personal media pages now return read-only snapshots, batch UI projection, reject stale history pages with a request version, share one cancellable add-to-download loop, and stop immediately when directory selection is canceled.
