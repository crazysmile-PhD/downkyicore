...
- Replace `SettingsManager.Instance` with an injected `ISettingsStore`, immutable validated snapshots, schema migration, debounced atomic writes, and explicit async shutdown flush; App/shell, migrated ViewModels/dialogs, account session, settings pages, and every download backend now share injected settings and diagnostic owners, while 10 production consumers still reference the singleton directly.
