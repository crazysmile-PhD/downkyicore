...
- Replace the remaining static `LogManager` usage with injected `Microsoft.Extensions.Logging`; Prism/Host composition, App crash/shutdown, About log management, and the complete download runtime/file-lifecycle/maintenance path are migrated, with download task scopes using redacted short IDs; 40 production caller files still contain executable static logger references.
