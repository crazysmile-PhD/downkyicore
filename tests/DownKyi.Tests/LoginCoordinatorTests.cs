...
public sealed class LoginCoordinatorTests
{
    ...
    public async Task SuccessfulLoginPersistsMergedCookiesThatValidateAfterReload()
    {
        ...
        var client = new TestBilibiliApiClient
        {
            GetStringAsyncHandler = (request, _) =>
            {
                ...
                var isLogin = cookieHeader.Contains("SESSDATA=fixture-session", StringComparison.Ordinal)
        ...
        var result = await coordinator.SaveLoginCookiesAsync(
            pollResult,
            new Uri("https://passport.bilibili.com/callback?ticket=fixture-ticket"),
    ...
    private sealed class StubLoginSessionFactory(StubLoginSession session)
        : IBilibiliLoginSessionFactory
    {
