...
Mux 失敗不等於來源損壞。`FfmpegOperationResult` 必須保留 input access、confirmed corruption、process unavailable/failure、timeout、output invalid、destination conflict/access 與 output IO 等 typed failure；`FfmpegProcessor` 只在 fail-on-error decode 的 stderr 含有明確媒體解碼損壞證據時回報可撤銷的 input。單純已啟動且非零 exit code、`File.Exists == false` 或目的檔寫入失敗都不足以判定來源損壞。多段 DURL concat 失敗後必須逐段診斷，正式 mux、concat 與失敗診斷共用同一個 `FfmpegMaxParallelJobs` gate。`DownloadTransferFileCleanup` 回傳 typed outcome；只有實體來源與 sidecar 清理成功的 key，才能由 Application service 在一次 durable mutation 中撤銷 completed state 與 backend identity。清理失敗的來源保留 durable retry state，Coordinator、MediaStage 與 MuxStage 都必須 fail closed。
