...
- `DownKyi.Application` 已擁有 Bilibili HTTP/buvid/cookie、logging 與 output identity contracts；`DownKyi.Infrastructure` 已擁有 async `IHttpClientFactory` transport、single-flight buvid provider、SQLite、write-behind、physical output identity adapter，以及私有 NLog logging sink、retention 與 diagnostic exporter。aria2、FFmpeg 與其餘 file system ownership 尚待後續切片。
