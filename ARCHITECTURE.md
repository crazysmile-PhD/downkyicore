...
Backend completion 不是 transfer success。Built-in 必須使用 Downloader 的完成事件與 byte counts；aria2 必須使用 final `tellStatus` 的 exact total/completed bytes、piece bitfield 與 piece count。兩者都只在廉價的 final file-length、unfinished-sidecar 與 bounded error-payload sanity check 通過後回傳 `Succeeded`；不建立 DownKyi 自有 segment ledger，也不預設全檔 hash。DURL manifest 必須在第一個 segment transfer 前拒絕非正或重複的 `Order` 與無可用地址的 segment，然後只依 `Order` 排序；不得自行增加 gap 或 contiguity 規則。
